...
class GitHubJobsTest < Minitest::Test
  ...
  def test_client_search_by_location
    stub_request_for('location=ny')
  ...
  def test_client_search_by_location_and_keyword
    stub_request_for('description=react&location=ny')
    client = GitHubJobs::Client.new
    response = client.search_by_location_and_keyword(keyword: 'react', location: 'ny')
    assert_equal 200, response.code
